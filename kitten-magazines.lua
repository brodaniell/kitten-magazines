...
local function sameTeam(character)
	...
	if character then
		...
		if Teams:FindFirstChild("Vaktovians") or Teams:FindFirstChild("VACs") then
			if target and target.Team == Teams.Vaktovians or target.Team == Teams.VACs and LocalPlayer.Team == Teams.Vaktovians or LocalPlayer.Team == Teams.VACs then
				return true
			end
		end
...
local function aimbot()
	...
	local mousePos = getMousePosition()
	local aimLock = (Toggles.Aimlock.Value and true or (target and not IgnoredPlayers[target.Name]))
